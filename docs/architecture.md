...
External access is disabled by default. When the user explicitly selects OpenAI or DeepSeek, the installer adds one provider-specific local stdio MCP server. Its only tool sends a bounded prompt to that provider's Responses API. The standard-library bridge reads `OPENAI_API_KEY` or `DEEPSEEK_API_KEY` from its process environment and stores only model and effort names in `.mcp.json`.

The tool deliberately has no filesystem functions. Its input contains an exact task, allowed repository-relative paths, constraints, and context selected by the owner. Combined DeepSeek payload text is capped before any network call. Output is untrusted proposal text, preferably a unified diff. The native Claude implementer reviews and applies accepted changes, so workspace mutation still has one writer. Provider access does not bypass normal verification, candidate freezing, review, or user-authority rules.
