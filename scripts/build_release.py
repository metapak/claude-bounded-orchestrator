...
def write_archive(path: Path, members: list[Path], *, windows: bool, start_file: tuple[str, str] | None = None) -> None:
    with zipfile.ZipFile(path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for source in members:
            ...
            mode = 0o755 if relative in {"setup.command", "scripts/install.sh", "scripts/install.py", "scripts/build_release.py", "scripts/validate.py", ".claude/tools/task_ledger.py", ".claude/tools/usage_report.py", ".claude/tools/local_eval.py", ".claude/tools/openai_mcp.py", ".claude/tools/deepseek_mcp.py"} else 0o644
